...
*/
		...
		ImGui::SetWindowSize(ImVec2(369, 479), ImGuiCond_FirstUseEver);
		...
		if (showMenu)
		{
			...
			if (ImGui::Begin(xorstr("thisisrico.de | Free Open SRC"), 0, ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_::ImGuiWindowFlags_NoResize))
			...
			{ 
					...
					ImGui::SetCursorPosX(ImGui::GetWindowWidth() - (ImGui::CalcTextSize(xorstr("Aimbot"), NULL, TRUE).x + ImGui::GetWindowWidth()) / 2);
					ImGui::Text(xorstr("Aimbot"));
					ImGui::SeparatorRainbow(color_red, color_green, color_blue);
					bool clickedH = ImGui::Checkbox("Memory", &Settings.SilentAimbot);
					...
					ImGui::SeparatorRainbow(color_red, color_green, color_blue);
					ImGui::SetCursorPosX(ImGui::GetWindowWidth() - (ImGui::CalcTextSize(xorstr("ESP"), NULL, TRUE).x + ImGui::GetWindowWidth()) / 2);
					...
					ImGui::Checkbox("Real Player ESP", &Settings.realplayertags);

					ImGui::Checkbox("Box ESP", &Settings.ESP.Boxes);

					ImGui::Checkbox("Player Lines ESP", &Settings.ESP.PlayerLines);

					ImGui::Checkbox("Player Names ESP", &Settings.ESP.PlayerNames);

					ImGui::Checkbox("BOT ESP", &Settings.botesp);

					ImGui::Checkbox(xorstr("Weapon ESP##checkbox"), &Settings.ESP.Weapons);
					...
					ImGui::Checkbox(xorstr("Lama ESP ##checkbox"), &Settings.ESP.LLamas);

					ImGui::Checkbox(xorstr("Player Weapons ESP ##checkbox"), &Settings.ESP.PlayerWeapons);

					ImGui::Checkbox(xorstr("Vehicle ESP ##checkbox"), &Settings.ESP.Vehicles);

					ImGui::Checkbox(xorstr("Player Ammo ESP"), &Settings.ESP.PlayerAmmo);

					ImGui::SeparatorRainbow(color_red, color_green, color_blue);
					ImGui::SetCursorPosX(ImGui::GetWindowWidth() - (ImGui::CalcTextSize(xorstr("misc"), NULL, TRUE).x + ImGui::GetWindowWidth()) / 2);
					ImGui::Text(xorstr("Exploits"));
					ImGui::SeparatorRainbow(color_red, color_green, color_blue);
					ImGui::Checkbox("Speed Hack", &Settings.CustomSpeedHack);

				    ImGui::Checkbox("Rapidfire", &Settings.VehicleBoost);

					ImGui::Checkbox("Crosshair ( Rainbow )", &Settings.Crosshair);
					
					ImGui::Checkbox("Crosshair ( Not Rainbow )", &Settings.Crosshair2);

					ImGui::Checkbox("No Clip", &Settings.killaura);

					ImGui::Checkbox("Fast Reload [R]", &Settings.FastReload);
					
					ImGui::Checkbox("No Spread", &Settings.NoSpreadAimbot);

					ImGui::Checkbox("Airstuck", &Settings.AirStuck);
					
					ImGui::Checkbox("Boat Fly", &Settings.TeleportToEnemies);

					ImGui::SliderFloat(xorstr("Fly (Boat Speed)"), &Settings.BoatFlySpeed, 200, 400, xorstr("%.2f"));

					ImGui::Checkbox("Wallhack [5m]", &Settings.IsBulletTeleporting);
					
					ImGui::Checkbox("Visible Check", &Settings.VisibleCheck);

					ImGui::SeparatorRainbow(color_red, color_green, color_blue);
					ImGui::SetCursorPosX(ImGui::GetWindowWidth() - (ImGui::CalcTextSize(xorstr("sliders"), NULL, TRUE).x + ImGui::GetWindowWidth()) / 2);
					ImGui::Text(xorstr("Sliders"));
				    ImGui::SeparatorRainbow(color_red, color_green, color_blue);
					ImGui::Text("Aimbot Smooth ");
					ImGui::SliderFloat(xorstr("##Slider"), &Settings.AimbotSlow, 0, 360, xorstr("Smooth: %.2f"));
					ImGui::Text("Aimbot FOV ");
					ImGui::SliderFloat(xorstr("##Slider2"), &Settings.AimbotFOV, 0, 2000, xorstr("FOV: %.2f"));
					ImGui::Text("ESP Distance ");
					ImGui::SliderFloat(xorstr("##Slider3"), &Settings.espdistancee, 0, 1000, xorstr("ESP Distance: %.2f"));
					ImGui::Text("FOV Changer ");
					ImGui::SliderFloat(xorstr("##Slider4"), &Settings.FOV, 50, 160, xorstr("FOV: %.2f"));
					ImGui::Text("Speed Hack Speed ");
					ImGui::SliderFloat(xorstr("##Slider5"), &Settings.CustomSpeedValue, 50, 400, xorstr("Speedhack Speed: %.2f"));


					ImGui::SeparatorRainbow(color_red, color_green, color_blue);
					ImGui::SetCursorPosX(ImGui::GetWindowWidth() - (ImGui::CalcTextSize(xorstr("Keys and Combo"), NULL, TRUE).x + ImGui::GetWindowWidth()) / 2);
					ImGui::Text(xorstr("Keys and Combo"));
					ImGui::SeparatorRainbow(color_red, color_green, color_blue);

					ImGui::Hotkey(xorstr("Speed Hack Key"), &Settings.CustomSpeedKeybind);
					ImGui::Hotkey(xorstr("Airstuck Key"), &Settings.AirstuckKey);
					ImGui::Hotkey(xorstr("Aimbot Key"), &Settings.AimKey);
					...
					{
						system("start https://discord.gg/gKFZ86rFXm");
					}
					if (ImGui::Button("Website"))
					{
						system("start https://thisisrico.de/");
					}
...
*/
	...
	HRESULT __stdcall hk_present(IDXGISwapChain* pSwapChain, UINT sync_interval, UINT flags)
	{
		...
		if (!device) {
			...
			if (m_pFont == NULL)
			{
				MessageBoxA(0, xorstr("couldn't find the font."), xorstr("MysticFN"), MB_ICONERROR);
				exit(0);
			}
		...
		window.DrawList->AddCircle(ImVec2(width / 2, height / 2), Settings.AimbotFOV2, ImGui::GetColorU32({ color_red, color_green, color_blue, 255 }), 128);
		...
		if (Settings.Crosshair2)
		{
			DrawLine(width / 2 - Settings.CrosshairScale, height / 2 - 0, width / 2 + Settings.CrosshairScale, height / 2 + 0, ImVec4(128, 224, 0, 200), Settings.CrosshairThickness);
			DrawLine(width / 2 + 0, height / 2 - Settings.CrosshairScale, width / 2 - 0, height / 2 + Settings.CrosshairScale, ImVec4(128, 224, 0, 200), Settings.CrosshairThickness);

		}

	
		auto FpsColor = ImGui::GetColorU32({ color_red, color_green, color_blue, 255 });
		DrawNormalText(m_pFont, TextFormat(xorstr("open src by thisisrico.de"), ImGui::GetIO().Framerate), ImVec2(20, 20), 19.0f, FpsColor, false);
					...
					else if (wcsstr(name.c_str(), xorstr(L"FortPickupAthena")))
					{
						...
						if (ChestRoot) {
							...
							if (Util::WorldToScreen(width, height, &ChestPos.X)) {
								...
								if (dist < 100)
								{
									DrawOutlinedText(m_pFont, TextFormat(xorstr("Chest thisisrico.de [%.0f m]"), dist), ImVec2(ChestPos.X, ChestPos.Y), 14.5f, ImGui::GetColorU32({ 205, 230, 0, 255 }), true);
								}
							}
						}
					}
						...
						if (LLamaRoot) {
							...
							if (Util::WorldToScreen(width, height, &LLamaPos.X)) {
								...
								if (dist < 5000)
								{
									DrawOutlinedText(m_pFont, TextFormat(xorstr("Gay Lama [%.0f m]"), dist), ImVec2(LLamaPos.X, LLamaPos.Y), 14.5f, ImGui::GetColorU32({ 210, 0, 240, 255 }), true);
								}
							}
						}
					}
						...
						if (Bot) {
							...
							if (Util::WorldToScreen(width, height, &Botroot.X)) {
								...
								{                                  //ImGui::GetColorU32({ color_red, color_green, color_blue, 255 });
									DrawOutlinedText(m_pFont, TextFormat(xorstr("Bo.t [%.0f m]"), dist), ImVec2(Botroot.X, Botroot.Y), 14.5f, ImGui::GetColorU32({ 255, 255, 255, 255 }), true);
								}
							}
						}
					}
						...
						if (Bot) {
							...
							if (Util::WorldToScreen(width, height, &Botroot.X)) {
								...
								if (dist < 5000)
								{
									DrawOutlinedText(m_pFont, TextFormat(xorstr("Real FN Kid [%.0f m]"), dist), ImVec2(Botroot.X, Botroot.Y), 14.5f, ImGui::GetColorU32({ 255, 0, 255 , 255 }), true);
								}
							}
						}
					}
...
*/
	...
	BOOLEAN Initialize()
	{
		...
		if (FAILED(D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, 0, 0, &featureLevel, 1, D3D11_SDK_VERSION, &sd, &swapChain, &device, nullptr, &context))) {
			MessageBox(0, xorstr(L"failed: dx11"), xorstr(L"MysticFN"), MB_ICONERROR);
			return FALSE;
		}
